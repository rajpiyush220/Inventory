...
public class StockManagementController extends BaseController {
  ...
  public String getAll(
    ...
    Sort sort =
        Sort.by(Sort.Direction.fromString(sortOrder), STOCK_SORT_COLUMN_MAP.get(sortColumn));
  ...
  public String getStockAudits(
    ...
    sortOrder = sortOrder.toUpperCase();
    Sort sort =
        Sort.by(Sort.Direction.fromString(sortOrder), STOCK_AUDIT_SORT_COLUMN_MAP.get(sortColumn));
    Pageable pageable = PageRequest.of(currentPage - 1, pageSize, sort);

    Page<Object[]> productPage = stockAuditService.getListData(pageable, searchType, searchKey);
